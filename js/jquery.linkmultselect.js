...
;(function($){
	...
	$.linkmultselect = {version: "1.5.10"};
	
	$.fn.linkmultselect = function(options) {
		var method = typeof arguments[0] == "string" && arguments[0];
		var args = method && Array.prototype.slice.call(arguments, 1) || arguments;

		// if a method is supplied, execute it for non-empty results
		if( method && this.length ){
			// get a reference to the first linkmultselect found
			var self = $.data(this[0], "linkmultselect");
			...
			// if method is defined, run it and return either it's results or the chain
				...
				this.each(function (i){
					...
					var r = $.data(this, "linkmultselect")[method].apply(self, args);
		...
		// initializing request
			...
			// create a new linkmultselect for each object found
			return this.each(function (){
				new $.linkmultselect(this, options);
			});
		};
	};

	var gid = 0;
	$.linkmultselect = function (el, options){
		options = $.extend({}, $.linkmultselect.defaults, options);
		
		// if no style defined, use default
		if( options.style.length == 0 ) options.style = $.linkmultselect.defaults.style;
		...
		this.change = function (){
			getSelectedItem().trigger("click.linkmultselect", [true, true]);
		...
		this.open = function (callback, bDoFocus){
			...
			// close any open linkmultselects
			$(document).triggerHandler("click.linkmultselect");
			// place focus
		...
		this.replaceOptions = function (options, includeFilter, doCallback){
			...
			repaint();
			// update the selected value
			getSelectedItem().trigger("click.linkmultselect", [true, doCallback]);
		};
		...
		$.data($input[0], "linkmultselect", this);
		...
		$ul
			...
			.delegate("li", "click.linkmultselect", function (e, nofocus, doCallback){
		...
		$a.bind("click.linkmultselect", function(e){
			e.preventDefault();
			//toggleOptions();
			// make sure the link has focus for IE6 & Chrome
			setTimeout(function (){
				$a.trigger("focus.linkmultselect");
			}, 0);
		})
		.bind("focus.linkmultselect", function (e, nofocus){
		...
		.bind("blur.linkmultselect", function (e){
		...
		.bind(($.browser.safari ? "keydown" : "keypress" ) + ".linkmultselect", function(e, e2){
			...
			switch(key) {
				...
				case 13: // return
					...
					if( $container.is(":visible") ){
						$container.find('li.' + options.classes.current).trigger('click.linkmultselect');
					} else {
						$a.trigger('click.linkmultselect');
					}
					break;
				case 9: // tab
				case 27: // escape
			...
			if( typeof shortcuts[cur_char] != "undefined" ){
				...
				$ul.find('#' + self.id + '_li_' + shortcuts[cur_char][current_char_pos]).trigger("click.linkmultselect");
				e.preventDefault();
				e.stopPropagation();
		...
		if( $.browser.msie ) $a.bind("keydown.linkmultselect", function (e){
			// check to see if a key was pressed that IE6 doesn't trigger a keypress event for
			if( ",8,9,33,34,35,36,37,38,39,40,".indexOf("," + e.keyCode + ",") > -1 ) return $(this).triggerHandler("keypress.linkmultselect", [e]);
		});

		function registerOpenEvents(){
			// we need to close the menu if we don't click on it
			$(document).bind("click.linkmultselect-" + cid, function (e){
				var $t = $(e.target);
			...
			$(window).bind("resize.linkmultselect-" + cid, function (){
				anchorTo($a, $container);
			});
		}

		function unregisterOpenEvents(){
			$(document).unbind("click.linkmultselect-" + cid);
			$(window).unbind("resize.linkmultselect-" + cid);
		}
		...
		function setPlaceHolder(){
			if( placeholder ) $ul.find("li." + options.classes.placeholder).trigger("click.linkmultselect");
		}
		...
		function setSelectedItem(value, doCallback){
			...
			// return the selected item			
			return $selected.trigger("click.linkmultselect", [true, doCallback]);
		}
		...
		function moveTo(pos, allowPlaceholder){
			...
			if( $container.is(":visible") ){
				...
				if( $current[0] !== $next[0] ) $next.trigger("click.linkmultselect");
			}
		}
...
*/			
	...
	$.linkmultselect.defaults = {
		  style: "linkmultselect"       // the default style to use
		...
		, init: null                // callback that occurs when a linkmultselect menu is initialized
